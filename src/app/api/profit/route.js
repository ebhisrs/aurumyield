...
export async function POST(request) {
  ...
  eligible.forEach(user => {
    ...
    global.__transactions.push({ id: txId, userId: user.id, type: 'performance', desc: `Monthly profit — ${program} ${percentage}%`, amount: profit, balanceBefore, balanceAfter: user.balance, status: 'Paid', date: new Date().toISOString().split('T')[0] });
  });

  global.__profitBatches.push({ id: batchId, program, percentage: Number(percentage), clients: eligible.length, totalProfit, note: note || '', admin: auth.username || 'Admin', date: new Date().toISOString().split('T')[0] });
  global.__audit.unshift({ action: `Posted ${percentage}% profit to ${eligible.length} ${program} clients. Total: $${totalProfit.toLocaleString()}. Note: ${note || 'N/A'}`, admin: auth.username || 'Admin', date: new Date().toISOString(), ip: '0.0.0.0' });
