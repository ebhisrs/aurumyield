...
export async function GET(request) {
  ...
  if (auth.role === 'admin') return NextResponse.json(global.__deposits);
...
export async function POST(request) {
  ...
  if (auth.role === 'admin' && data.action) {
    ...
    if (data.action === 'approve') {
      ...
      global.__audit.unshift({ action: `Approved deposit ${dep.ref} for ${user?.name}; +$${dep.amount.toLocaleString()}`, admin: 'Admin', date: new Date().toISOString(), ip: '0.0.0.0' });
    } else if (data.action === 'reject') {
      dep.status = 'rejected';
      global.__audit.unshift({ action: `Rejected deposit ${dep.ref}`, admin: 'Admin', date: new Date().toISOString(), ip: '0.0.0.0' });
    }
