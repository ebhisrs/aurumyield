...
// GET — list admins, settings, full audit
export async function GET(request) {
  ...
  if (type === 'admins') {
    const admins = global.__admins.map(({ password, ...a }) => a);
  ...
  return NextResponse.json({ error: 'Invalid type. Use: admins, settings, audit, stats' }, { status: 400 });
}

// POST — create admin, update settings, update admin, delete admin
export async function POST(request) {
  ...
  if (data.action === 'create_admin') {
    ...
    global.__admins.push({
      id, username, name, password: bcrypt.hashSync(password, 10), role: 'admin', status: 'active',
      permissions: permissions || ['approve_clients','approve_deposits','approve_withdrawals','post_profit'],
  ...
  // Update admin status or permissions
  if (data.action === 'update_admin') {
    ...
    if (data.status) {
      ...
      global.__audit.unshift({ action: `Super Admin ${data.status === 'active' ? 'enabled' : 'disabled'} admin: ${admin.username}`, admin: sa.username, date: new Date().toISOString(), ip: '0.0.0.0' });
    }
    if (data.permissions) {
      admin.permissions = data.permissions;
      global.__audit.unshift({ action: `Super Admin updated permissions for: ${admin.username} → [${data.permissions.join(', ')}]`, admin: sa.username, date: new Date().toISOString(), ip: '0.0.0.0' });
    }
    if (data.newPassword) {
      admin.password = bcrypt.hashSync(data.newPassword, 10);
      global.__audit.unshift({ action: `Super Admin reset password for: ${admin.username}`, admin: sa.username, date: new Date().toISOString(), ip: '0.0.0.0' });
    }
    return NextResponse.json({ ok: true });
  }
  ...
  if (data.action === 'update_settings') {
    ...
    const oldSettings = { ...global.__settings };
    Object.assign(global.__settings, settings);
    const changed = Object.keys(settings).filter(k => String(oldSettings[k]) !== String(settings[k]));
    if (changed.length > 0) {
      global.__audit.unshift({ action: `Super Admin updated settings: ${changed.join(', ')}`, admin: sa.username, date: new Date().toISOString(), ip: '0.0.0.0' });
    }
